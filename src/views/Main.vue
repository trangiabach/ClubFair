...
<div>
  <div class = 'menu container-fluid'>
    ...
    <div class = 'ntt minverva-university'><a target="_blank"  href = '/minerva'>Khám phá cơ hội học tập và làm việc ở 7 quốc gia trong 4 năm Đại học</a></div>
    </section>
  ...
  <div class = 'register' id = 'register-mobile'>Đăng ký</div>
...
<div class = 'container-fluid ntt-info minverva-university-ntt'>
  ...
  <div class="login-overlay" v-if='register'></div>
  <div class="form-submit" v-show='register'>
    ...
    <div class ='cancel-form-menu'><svg class="svg-list" viewBox="0 0 20 20">
      <path fill="#e1144b" d="M15.898,4.045c-0.271-0.272-0.713-0.272-0.986,0l-4.71,4.711L5.493,4.045c-0.272-0.272-0.714-0.272-0.986,0s-0.272,0.714,0,0.986l4.709,4.711l-4.71,4.711c-0.272,0.271-0.272,0.713,0,0.986c0.136,0.136,0.314,0.203,0.492,0.203c0.179,0,0.357-0.067,0.493-0.203l4.711-4.711l4.71,4.711c0.137,0.136,0.314,0.203,0.494,0.203c0.178,0,0.355-0.067,0.492-0.203c0.273-0.273,0.273-0.715,0-0.986l-4.711-4.711l4.711-4.711C16.172,4.759,16.172,4.317,15.898,4.045z"></path>
    </svg></div>
  </div>
    ...
    initialize() {
      ...
      class Webgl {
          ...
          document.getElementsByClassName('login-btn')[0].addEventListener('click', async function() {
            if(that.user.name != '' && that.user.class != '' && that.user.school != ''  && that.user.email != ''  && that.user.phone != '') {
              ...
              await users.add(that.user).then(res => {
                gsap.to('.form-submit', {opacity: 0, duration: 1.2, ease: Power4.easeOut, onComplete: function() {
                  // that.hasLogin = true
                  // that.$store.commit('setAuthentication', true)
                  //  loading_tl.pause()
                    document.getElementsByClassName('form-submit')[0].style.opacity = 1
                    document.getElementsByClassName('form-submit')[0].style.top = 'initial'
                    that.register = false
                    that.user.name = ''
                    that.user.class = ''
                    that.user.school = ''
                ...
                gsap.to('.login-overlay', {opacity: 0, duration: 0.5, ease: Power4.easeOut, onComplete: function() {
                  // that.hasLogin = true
                  // that.$store.commit('setAuthentication', true)
        ...
        moveToFirstLevel() {
          ...
          document.getElementsByClassName('center-title')[1].addEventListener( 'pointerdown', () => {
          ...
          document.getElementsByClassName('center-title-small')[0].addEventListener( 'pointerdown', () => {
            this.cards.art.animateFromCenterToCluster(this.cards.art)
            this.cards.academic.animateFromCenterToCluster(this.cards.academic)
            this.cards.sports.animateFromCenterToCluster(this.cards.sports)
            this.cards.social.animateFromCenterToCluster(this.cards.social)
...
`
        ...
        addThreeEL() {
          ...
          document.getElementsByClassName('right-header-1')[0].addEventListener('click', function() {
          ...
          document.getElementsByClassName('register')[1].addEventListener('click', function() {
           ...
           document.getElementsByClassName('cancel-form-menu')[0].addEventListener('click', function() {
                that.register = false
              })
  ...
  mounted() {
    ...
    this.hasLogin = true
     if(window.innerWidth < 500) {
                document.getElementById('register-mobile').style.opacity = 1
                document.getElementsByClassName('register')[1].style.opacity = 0
              } else {
                document.getElementById('register-mobile').style.opacity = 0
                document.getElementsByClassName('register')[1].style.opacity = 1
              }
